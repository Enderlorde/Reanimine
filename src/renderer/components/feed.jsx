...
const Feed = (props) => {
  ...
  return (
    <ul className={`${feed_styles.feed} ${props.className}`}>
      {data.map((item, index) => (
        <li
          className={`${feed_styles.item} ${backplate_styles.backplate}`}
          key={index}>
          {item.playPageImage && (
            <div className={`${feed_styles.img}`}>
              <LazyLoadImage
                ...
                src={
                  "https://launchercontent.mojang.com" + item.playPageImage.url
                }
              />
